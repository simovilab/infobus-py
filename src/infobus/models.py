...
class RealtimeData(BaseInfobusModel):
    ...
    scheduled_arrival: Optional[datetime] = Field(
        None, description="Scheduled arrival time"
    )
    ...
    bearing: Optional[float] = Field(
        None, ge=0, le=360, description="Vehicle bearing in degrees"
    )
...
class Route(BaseInfobusModel):
    ...
    route_short_name: Optional[str] = Field(
        None, description="Short name for the route"
    )
...
class Screen(BaseInfobusModel):
    ...
    refresh_interval: Optional[int] = Field(
        None, description="Data refresh interval in seconds"
    )
    display_routes: Optional[List[str]] = Field(
        None, description="Route IDs to display"
    )
...
class Alert(BaseInfobusModel):
    ...
    description_text: Optional[str] = Field(
        None, description="Detailed alert description"
    )
    url: Optional[str] = Field(None, description="URL for more information")

    # Timing information
    active_period_start: Optional[datetime] = Field(
        None, description="Alert start time"
    )
    ...
    affected_agencies: Optional[List[str]] = Field(
        None, description="Affected agency IDs"
    )

    # Severity and priority
    severity_level: Optional[str] = Field(None, description="Alert severity level")
    priority: Optional[int] = Field(
        None, ge=1, le=5, description="Alert priority (1=highest)"
    )
...
class ServiceStatus(BaseInfobusModel):
    ...
    uptime_percentage: Optional[float] = Field(
        None, ge=0, le=100, description="Service uptime percentage"
    )
