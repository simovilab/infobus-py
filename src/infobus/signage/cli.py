...
def generar_señal(template_name, output_format, output, data, dpi=300):
    ...
    try:
        ...
        if output:
            with open(output, modo_archivo, encoding='utf-8' if output_format == 'svg' else None) as f:
                f.write(resultado)
            click.echo(f"✅ Señal guardada en: {output}")
        else:
            nombre_default = f"señal_{template_name}.{output_format}"
            with open(nombre_default, modo_archivo, encoding='utf-8' if output_format == 'svg' else None) as f:
                f.write(resultado)
...
# COMANDO STOP
@signage.command()
@click.option('--template', default='stop_vertical', 
...
def stop(template, output_format, size, dpi, theme, lang, qr_url, output, stop_name, stop_code):
...
# COMANDO VEHICLE
@signage.command()
@click.option('--template', default='vehicle_interior', 
...
def vehicle(template, output_format, size, dpi, theme, lang, qr_url, output, route_number, destination, accessibility):
...
# COMANDO STATION
@signage.command()
@click.option('--template', default='station_modular', 
...
def station(template, output_format, size, dpi, theme, lang, qr_url, output, station_name, line_name):
    ...
    datos = {
        ...
        "line_name": line_name or "Línea Principal"
    }
...
@click.option('--route-number', required=True, help='Número de ruta (requerido)')
def route(template, output_format, size, dpi, theme, lang, qr_url, output, route_name, route_number):
...
@click.option('--template', required=True, 
              help='Plantilla a usar (requerido)')
@click.option('--format', 'output_format', 
              type=click.Choice(['svg', 'png', 'pdf']), 
              default='svg',
              help='Formato de salida')
